        ...
        const fetchData = async () => {
            try {
                ...
                const urls = [
                    ...
                    `https://api.themoviedb.org/3/movie/${id}/similar?api_key=${apiKey}&language=fr`, // Similar movies
                ];

                const allResponses = await Promise.all(urls.map(url => fetch(url)));
                const [movieData, reviewsData, creditsData, posterData, similarMoviesData] = await Promise.all(allResponses.map(response => response.json()));
                ...
                // Similar movies
                setSimilarMovies(similarMoviesData.results.slice(0, 10)); // Limiter à 10 résultats
